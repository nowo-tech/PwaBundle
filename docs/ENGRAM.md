...
- [Clearing or resetting the database](#clearing-or-resetting-the-database)
- [References](#references)

## What is Engram?

**Engram** is an [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) server that gives AI coding agents (Cursor, Claude Code, etc.) **persistent memory**. It stores context in a local database (SQLite) so the agent does not need to re-discover project structure and conventions in every session.
...
- **Entities & stats** — Inspect the knowledge graph and database statistics.
...
`engram init` will detect your editor, prompt for a Gemini API key (for embeddings and consolidation; free at [Google AI Studio](https://aistudio.google.com/apikey)), and create your database. You can skip or complete init; the MCP will work as long as the `engram` binary is in your `PATH`.
...
1. **Install** the Engram CLI as above and run `engram init` if you want to set up the database and Gemini key.
...
## Clearing or resetting the database
...
- **Empty the database completely:** The database is a local SQLite file. Default location: `~/.engram/default.db` (or the path set in `ENGRAM_DB`). To wipe all memories, delete that file or the whole directory:
  ```bash
  rm -f ~/.engram/default.db
  # or, to remove the whole database directory:
  rm -rf ~/.engram
  ```
  After that, the next time you use Engram a new database will be created. You can run `engram init` again if you want to reconfigure the Gemini key or editor.
